...
public class EvalVisitor extends CalculatorBaseVisitor<Double> {
    ...
    HashMap<String, CalculatorParser.ReturnTypeContext> functions_return = new HashMap<String, CalculatorParser.ReturnTypeContext>();
    ...
    public Double visitFuncCallTag(CalculatorParser.FuncCallTagContext ctx){
        ...
        CalculatorParser.ReturnTypeContext return_statement = this.functions_return.get(func_name);
        
        if(param_values.expr().size() == param_names.ID().size()){
            ...
            if(param_values != null && param_names != null){
                for(int i = 0; i < param_names.ID().size(); i++) {
                    this.current_scope.variables.put(param_names.ID().get(i).getText(), visit(param_values.expr(i)));
                }
            }
